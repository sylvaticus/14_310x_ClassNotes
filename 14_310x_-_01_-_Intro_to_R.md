...
Create a matrix:
`m = matrix(data = c(1:20), nrow = 5, ncol = 4, byrow = FALSE, dimnames = list(list("a","b","c","d","e"),list("A","B","C","D")))`
...
`dim(df)` Returns df rows and cols. Also available: `nrow(df)` and `ncol(df)`
...
This example:
`gather(students, sex, count, male, female)` (or `gather(students, sex, count, -grade)`)
