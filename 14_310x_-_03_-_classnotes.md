...
The paraemter bandwith is `bw`, e.g. `geom_density(kernel="gaussian", aes(height_cm), bw=2)`
...
Too small bandwith: under smoothing effect: the kernel distribution it's really trying to go to every nook
and cranny of the distribution.
...
Too large bandwitch: over smoothing effect. There seems to be missing something about the distribution that
is real, that is not noise.
...
The kernel and bandwitch choice can be obtained trying to minimize the mean square errors (the difference
between your estimated function minus the true squared), which
